...
def ranking(player_name, tempo_total):
    ...
    # Fontes padrão caso "Pixelify Sans" não esteja disponível
    try:
    ...
    while rodando:
        ...
        titulo = fonte.render("PONTUAÇÃO FINAL", True, AMARELO)
        tela.blit(titulo, (SCREEN_WIDTH // 2 - titulo.get_width() // 2, 30))

        for i, (nome, tempo) in enumerate(jogadores[:10]):
            linha_render = pequena.render(f"{i+1}º {nome} - {tempo} segundos", True, BRANCO)
            tela.blit(linha_render, (SCREEN_WIDTH // 2 - linha_render.get_width() // 2, 150 + i * 35))

        instrucoes = pequena.render("Aperte ENTER para voltar ao menu", True, PRETO)
        tela.blit(instrucoes, (SCREEN_WIDTH // 2 - instrucoes.get_width() // 2, 550))
        pygame.display.flip()
        clock.tick(60)
